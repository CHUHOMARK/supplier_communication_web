 ...
 * 先删除用户旧数据，再插入新数据
 */
export async function createPurchaseOrders(orders: InsertPurchaseOrder[]) {
  const db = await getDb();
  if (!db) throw new Error("Database not initialized");
 ...
 * 删除用户的所有采购订单
 */
export async function deletePurchaseOrdersByUserId(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not initialized");
...
// ==========================================
// Supplier Authentication & Portal Functions
// ==========================================

/**
 * Look up supplier by login code
 */
 ...
 * Verify supplier PIN
 */
export async function verifySupplierPin(supplierId: number, pin: string): Promise<boolean> {
  const db = await getDb();
  if (!db) return false;
  
  const result = await db
    .select({ defaultPin: suppliers.defaultPin })
    .from(suppliers)
    .where(eq(suppliers.id, supplierId))
    .limit(1);
  
  if (result.length === 0 || !result[0].defaultPin) return false;
  
  const bcrypt = await import('bcrypt');
  return bcrypt.default.compare(pin, result[0].defaultPin);
}

/**
 * Update supplier PIN
 */
export async function updateSupplierPin(supplierId: number, newPin: string, isFirstLogin: boolean) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  await db
    .update(suppliers)
    .set({ defaultPin: hashedPin, firstLogin: isFirstLogin })
    .where(eq(suppliers.id, supplierId));
}

/**
 * Mark supplier first login as done
 */
export async function markSupplierFirstLoginDone(supplierId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
/**
 * Create a supplier with auto-generated code and default PIN
 * Returns: { supplierId, supplierCode, defaultPin }
 */
export async function createSupplierWithCode(data: {
  userId: number;
  supplierName: string;
  ...
  phone?: string;
  notes?: string;
  defaultPin?: string; // optional, defaults to "888888"
}) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  // Generate supplier code: SUP-XXXXX (random 5 digits)
  const randomNum = Math.floor(10000 + Math.random() * 90000);
  const supplierCode = `SUP-${randomNum}`;
  
  // Default PIN
  const pin = data.defaultPin || "888888";
  const bcrypt = await import('bcrypt');
  const hashedPin = await bcrypt.default.hash(pin, 10);
  
  const supplierId = await db.insert(suppliers).values({
    userId: data.userId,
...
/**
 * Get materials assigned to a supplier (across all plans)
 */
export async function getSupplierMaterials(supplierId: number) {
  ...
  const mappings = await db
    .select({
      materialCode: materialSupplierMappings.materialCode,
      planId: materialSupplierMappings.planId,
      sharePercentage: materialSupplierMappings.sharePercentage,
      priority: materialSupplierMappings.priority,
    })
    .from(materialSupplierMappings)
    .where(eq(materialSupplierMappings.supplierId, supplierId));
  
  if (mappings.length === 0) return [];
  
  // Get unique planIds
  const planIds = Array.from(new Set(mappings.map(m => m.planId)));
  
  // Get all material items for these plans
  const items = await db
    .select()
    .from(materialItems)
    .where(inArray(materialItems.planId, planIds));
  
  // Get plans
  const plans = await db
    .select()
    .from(materialPlans)
    .where(inArray(materialPlans.id, planIds));
  
  // Build result
  const itemMap = new Map<string, typeof items[0]>();
  for (const item of items) {
    itemMap.set(item.materialCode, item);
  }
  
  const planMap = new Map<number, typeof plans[0]>();
  for (const plan of plans) {
    planMap.set(plan.id, plan);
  }
  
  const result = mappings.map(m => {
    const item = itemMap.get(m.materialCode);
    const plan = planMap.get(m.planId);
    return {
      materialCode: m.materialCode,
      planId: m.planId,
      planName: plan?.fileName || `计划 #${m.planId}`,
...
/**
 * Get supplier's confirmation records with delivery progress
 */
...
/**
 * Update supplier's production status for a confirmation
 */
export async function updateSupplierProductionStatus(
  ...
  await db
    .update(supplierConfirmations)
    .set({ productionStatus: productionStatus as any })
    .where(eq(supplierConfirmations.id, confirmationId));
}

/**
 * Update supplier's daily schedule (delivery quantities)
 */
export async function updateSupplierDailySchedule(
  confirmationId: number,
  dailySchedule: Record<string, number>
) {
  const db = await getDb();
  ...
  await db
    .update(supplierConfirmations)
    .set({ dailySchedule: dailySchedule as any })
    .where(eq(supplierConfirmations.id, confirmationId));
}
...
/**
 * Get supplier's pending confirmations count
 */
export async function getSupplierPendingConfirmations(supplierId: number) {
  const db = await getDb();
  if (!db) return 0;
  ...
  const result = await db
    ...
    .where(and(
      eq(supplierConfirmations.supplierId, supplierId),
      eq(supplierConfirmations.status, 'pending')
    ));
...
/**
 * Get supplier's upcoming deliveries (within next 7 days)
 */
export async function getSupplierUpcomingDeliveries(supplierId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  for (const conf of confirmations) {
    const dailySchedule = conf.dailySchedule as Record<string, number> || {};
    const plan = await db.select().from(materialPlans).where(eq(materialPlans.id, conf.planId)).limit(1);
    const planName = plan[0]?.fileName || `计划 #${conf.planId}`;
    
    for (const [dateStr, qty] of Object.entries(dailySchedule)) {
      const date = new Date(dateStr);
      if (date >= today && date <= sevenDaysFromNow && qty > 0) {
        upcoming.push({
...
/**
 * Get supplier's completion rate
 */
export async function getSupplierCompletionRate(supplierId: number) {
  const db = await getDb();
  if (!db) return { rate: 0, confirmed: 0, total: 0 };
  ...
  const confirmed = confirmations.filter(c => 
    c.status === 'confirmed' || c.status === 'modified'
  ).length;
...
/**
 * Get supplier notifications from confirmation status changes
 */
export async function getSupplierNotifications(supplierId: number, limit: number = 20) {
  const db = await getDb();
  if (!db) return [];
  ...
  const confirmations = await db
    .select({
      id: supplierConfirmations.id,
      status: supplierConfirmations.status,
      confirmedAt: supplierConfirmations.confirmedAt,
      createdAt: supplierConfirmations.createdAt,
      planId: supplierConfirmations.planId,
    })
    .from(supplierConfirmations)
    .where(eq(supplierConfirmations.supplierId, supplierId))
    .orderBy(desc(supplierConfirmations.confirmedAt))
    .limit(limit);
  ...
  for (const conf of confirmations) {
    ...
    if (conf.status === 'pending') {
      notifications.push({
        id: conf.id,
        type: 'plan_issued',
        title: `新计划已下发：${planName}`,
        content: '请及时确认物料计划和交期',
        createdAt: conf.createdAt,
        isRead: false,
      });
    } else if (conf.status === 'confirmed' || conf.status === 'modified') {
      notifications.push({
        id: conf.id,
        type: 'confirmed',
        title: `计划已确认：${planName}`,
        content: '您的确认已提交，请按时交货',
        createdAt: conf.confirmedAt || conf.createdAt,
        isRead: true,
      });
    }
  }
...
/**
 * Get supplier dashboard stats (summary for dashboard cards)
 */
export async function getSupplierDashboardStats(supplierId: number) {
  const [pendingCount, upcomingDeliveries, completionRate] = await Promise.all([
    getSupplierPendingConfirmations(supplierId),
    getSupplierUpcomingDeliveries(supplierId),
    getSupplierCompletionRate(supplierId),
  ]);
  ...
  return {
    ...
    earliestDelivery: upcomingDeliveries[0]?.date || null,
    completionRate: completionRate.rate,
    totalConfirmations: completionRate.total,
    confirmedCount: completionRate.confirmed,
  };
}
