...
export const appRouter = router({
  ...
  supplier: router({
    ...
    // 创建供应商并自动生成登录编号和默认PIN
    createWithLogin: protectedProcedure
      .input(z.object({
        supplierName: z.string(),
        contactPerson: z.string().optional(),
        email: z.string().email().optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await db.createSupplierWithCode({
          userId: ctx.user.id,
    ...
    importEmails: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        console.log('[importEmails] userId:', ctx.user.id, 'file:', input.filename, 'b64Len:', input.fileBase64?.length);
        try {
          const buffer = Buffer.from(input.fileBase64, 'base64');
          ...
          for (const row of data as any[]) {
            ...
            const supplierId = supplierMap.get(trimmedName);
            if (supplierId) {
              try {
                await db.updateSupplier(supplierId, { email: trimmedEmail });
                console.log('[importEmails] Updated:', trimmedName, '→', trimmedEmail);
              } catch (e) {
                console.error('[importEmails] Update failed:', trimmedName, e);
              }
              successList.push(trimmedName);
            } else {
              try {
                const newId = await db.createSupplier({
                  userId: ctx.user.id,
                  supplierName: trimmedName,
                  email: trimmedEmail,
                });
                supplierMap.set(trimmedName, Number(newId));
                console.log('[importEmails] Created:', trimmedName, '→', trimmedEmail, 'id:', newId);
              } catch (e) {
                console.error('[importEmails] Create failed:', trimmedName, e);
              }
              successList.push(trimmedName);
            }
          }
  ...
  purchaseOrder: router({
    ...
    parseAndCalculate: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 直接存储采购订单明细到 purchase_orders 表
        try {
          await db.deletePurchaseOrdersByUserId(ctx.user.id);
          const now = new Date();
          const poRecords = orders.map(o => ({
            userId: ctx.user.id,
            businessDate: now,
            poNumber: '',
            ...
            materialSpec: o.materialSpec || null,
            purchaseQuantity: o.quantity,
            confirmedQuantity: 0,
            receivedQuantity: 0,
            undeliveredQuantity: o.undeliveredQuantity,
            requiredDeliveryDate: o.deliveryDate || null,
            purchaseStaff: null,
          }));
          await db.createPurchaseOrders(poRecords);
          console.log('[parseAndCalculate] Stored', poRecords.length, 'PO records');
        } catch (e) {
          console.error('[parseAndCalculate] Failed to store PO data:', e);
        }
    ...
    applyCalculations: protectedProcedure
      .input(z.object({
        ...
        purchaseOrders: z.array(z.object({
          ...
          supplierName: z.string(),
          quantity: z.number(),
          undeliveredQuantity: z.number(),
          deliveryDate: z.string().optional(),
        })).optional(),
        createMissingSuppliers: z.boolean().default(true),
      }))
      .mutation(async ({ ctx, input }) => {
        console.log('[applyCalculations] userId:', ctx.user.id, 'planId:', input.planId, 'calculations:', input.calculations.length, 'purchaseOrders:', input.purchaseOrders?.length || 0);
        const existingSuppliers = await db.getSuppliersByUserId(ctx.user.id);
        ...
        // 存储采购订单明细到 purchase_orders 表
        console.log('[applyCalculations] purchaseOrders count:', input.purchaseOrders?.length || 0);
        if (input.purchaseOrders && input.purchaseOrders.length > 0) {
          await db.deletePurchaseOrdersByUserId(ctx.user.id);
          const now = new Date();
          const poRecords = input.purchaseOrders.map(po => ({
            userId: ctx.user.id,
            businessDate: now,
            poNumber: '',
            ...
            confirmedQuantity: 0,
            receivedQuantity: 0,
            undeliveredQuantity: po.undeliveredQuantity,
            requiredDeliveryDate: po.deliveryDate || null,
            purchaseStaff: null,
          }));
          await db.createPurchaseOrders(poRecords);
        }
  ...
  confirmation: router({
    ...
    getByToken: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const supplierMappings = mappings.filter(m => m.supplierId === confirmation.supplierId);
        const supplierMaterialCodes = new Set(supplierMappings.map(m => m.materialCode));
        
        const shareMap = new Map<string, string>();
        supplierMappings.forEach(m => shareMap.set(m.materialCode, m.sharePercentage || '100'));
        ...
        const undeliveredMap = new Map<string, number>();
        if (drizzleDb && supplier?.supplierName) {
          const undeliveredData = await drizzleDb
            .select({
              materialCode: poTable.materialCode,
              totalUndelivered: sqlFn`COALESCE(SUM(${poTable.undeliveredQuantity}), 0)`,
            })
            .from(poTable)
            .where(
              andFn(
                eqFn(poTable.userId, confirmation.userId),
                eqFn(poTable.supplierName, supplier.supplierName)
              )
            )
            .groupBy(poTable.materialCode);
          
          for (const row of undeliveredData) {
            undeliveredMap.set(row.materialCode, Number(row.totalUndelivered));
          }
        }

        // 计算交货计划：小量先交满，大量兜底（每天 ≤ 计划量，总量 ≤ 计划总量）
        function computeDeliverySchedule(
          planSchedule: Record<string, number>,
          undeliveredQty: number
        ): Array<{ date: string; quantity: number; planQuantity: number }> {
          if (undeliveredQty <= 0) return [];
          
          const planTotal = Object.values(planSchedule).reduce((s, q) => s + q, 0);
          // 实际可交数量 = min(未交数量, 计划总量)
          const toDeliver = Math.min(undeliveredQty, planTotal);
          
          // 按计划量从小到大排序，小量先满
          const sorted = Object.entries(planSchedule)
            .filter(([_, q]) => q > 0)
            .sort((a, b) => a[1] - b[1]);
          
          const tmp: Record<string, number> = {};
          let remaining = toDeliver;
          for (const [date, planQty] of sorted) {
            if (remaining <= 0) break;
            const qty = Math.min(planQty, remaining);
            tmp[date] = qty;
            remaining -= qty;
          }
          
          // 按日期顺序输出
          return Object.keys(planSchedule).sort()
            .filter(d => tmp[d])
            .map(d => ({ date: d, quantity: tmp[d], planQuantity: planSchedule[d] }));
        }
        ...
        const items = allItems
          ...
          .map(item => {
            ...
            // 缺数 × 份额 = 该供应商应交数量
            const materialShortage = Math.abs(parseFloat(item.shortage || '0') || 0);
            const undeliveredQty = Math.round(materialShortage * sharePercentage / 100);
            
            // 用原始计划量（份额前）做小量先满的分配基准
            const planTotal = Object.values(originalSchedule).reduce((s, q) => s + q, 0);
            const deliverySchedule = computeDeliverySchedule(originalSchedule, undeliveredQty);
            ...
            return {
              ...
              planSchedule: originalSchedule,
              deliverySchedule,
            };
          });

        // 查询修改记录
        const modifications = await db.getModificationsByConfirmationId(confirmation.id);

        return {
          confirmation: {
            id: confirmation.id,
            status: confirmation.status,
            productionStatus: confirmation.productionStatus || 'not_started',
            supplierNotes: confirmation.supplierNotes || '',
            confirmedAt: confirmation.confirmedAt,
            expiresAt: confirmation.expiresAt,
          },
          deliveryProgress: (() => {
            try { return confirmation.supplierResponse ? JSON.parse(confirmation.supplierResponse) : {}; } catch { return {}; }
          })(),
          modifications,
          supplier: supplier ? {
            id: supplier.id,
    ...
    // 更新单日交货进度
    updateDeliveryProgress: publicProcedure
      .input(z.object({
        token: z.string(),
        date: z.string(),
        status: z.enum(['not_started', 'material_prep', 'in_production', 'in_qc', 'ready_to_ship', 'shipped']),
      }))
      .mutation(async ({ input }) => {
        const confirmation = await db.getConfirmationByToken(input.token);
        if (!confirmation) {
          throw new TRPCError({ code: 'NOT_FOUND', message: '确认记录不存在' });
        }
        
        // 读取现有进度
        let progress: Record<string, string> = {};
        try { progress = confirmation.supplierResponse ? JSON.parse(confirmation.supplierResponse) : {}; } catch {}
        
        // 更新指定日期的进度
        progress[input.date] = input.status;
        
        // 写回
        const { getDb: getDbFn } = await import('./db');
        const drizzleDb = await getDbFn();
        if (drizzleDb) {
          const { eq: eqFn } = await import('drizzle-orm');
          const { supplierConfirmations } = await import('../drizzle/schema');
          await drizzleDb.update(supplierConfirmations)
            .set({ supplierResponse: JSON.stringify(progress) })
            .where(eqFn(supplierConfirmations.id, confirmation.id));
        }
    ...
    submitModifications: publicProcedure
      .input(z.object({
        ...
        productionStatus: z.enum(['not_started', 'material_prep', 'in_production', 'in_qc', 'ready_to_ship', 'shipped']).optional(),
      }))
      .mutation(async ({ input }) => {
        const confirmation = await db.getConfirmationByToken(input.token);
  ...
  supplierAuth: router({
    me: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        ...
        const supplier = await db.getSupplierById(decoded.supplierId);
        ...
        return {
          ...
          supplierName: supplier.supplierName,
          contactPerson: supplier.contactPerson,
          firstLogin: supplier.firstLogin,
        };
      } catch {
        return null;
      }
    ...
    login: publicProcedure
      .input(z.object({ supplierCode: z.string(), pin: z.string() }))
      .mutation(async ({ input, ctx }) => {
        const supplier = await db.getSupplierByCode(input.supplierCode);
        if (!supplier || !supplier.defaultPin) {
          throw new TRPCError({ code: 'UNAUTHORIZED', message: '供应商编号或PIN码错误' });
        }
        const isValid = await bcrypt.compare(input.pin, supplier.defaultPin);
        if (!isValid) {
          throw new TRPCError({ code: 'UNAUTHORIZED', message: '供应商编号或PIN码错误' });
        }
        const token = jwt.sign(
          { supplierId: supplier.id, supplierCode: supplier.supplierCode, role: 'supplier' },
          ENV.jwtSecret,
          { expiresIn: '30d' }
        );
        const cookieOptions = getSessionCookieOptions(ctx.req);
        ctx.res.cookie(SUPPLIER_COOKIE_NAME, token, { ...cookieOptions, maxAge: 30 * 24 * 60 * 60 * 1000 });
        return {
          success: true,
          firstLogin: supplier.firstLogin,
          supplier: {
    ...
    changePin: publicProcedure
      .input(z.object({ oldPin: z.string(), newPin: z.string().min(4).max(20) }))
      .mutation(async ({ input, ctx }) => {
        const cookies = ctx.req.headers.cookie;
        ...
        try {
          const token = cookieMatch[1];
          const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
          const isValid = await db.verifySupplierPin(decoded.supplierId, input.oldPin);
          if (!isValid) throw new TRPCError({ code: 'BAD_REQUEST', message: '原PIN码错误' });
          await db.updateSupplierPin(decoded.supplierId, input.newPin, false);
          return { success: true, message: 'PIN码已更新' };
        } catch (error) {
  ...
  supplierPortal: router({
    getInfo: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        ...
        const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
        const supplier = await db.getSupplierById(decoded.supplierId);
        if (!supplier) throw new TRPCError({ code: 'UNAUTHORIZED', message: '供应商不存在' });
        return {
          id: supplier.id,
          supplierCode: supplier.supplierCode,
          supplierName: supplier.supplierName,
    ...
    getDashboardStats: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        const token = cookieMatch[1];
        const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
        return await db.getSupplierDashboardStats(decoded.supplierId);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: 'UNAUTHORIZED', message: '未登录' });
      }
    ...
    getNotifications: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        const token = cookieMatch[1];
        const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
        return await db.getSupplierNotifications(decoded.supplierId);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: 'UNAUTHORIZED', message: '未登录' });
      }
    ...
    getMaterials: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        const token = cookieMatch[1];
        const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
        return await db.getSupplierMaterials(decoded.supplierId);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: 'UNAUTHORIZED', message: '未登录' });
      }
    ...
    getConfirmations: publicProcedure.query(async ({ ctx }) => {
      ...
      try {
        const token = cookieMatch[1];
        const decoded = jwt.verify(token, ENV.jwtSecret) as { supplierId: number };
        return await db.getSupplierConfirmations(decoded.supplierId);
      } catch (error) {
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({ code: 'UNAUTHORIZED', message: '未登录' });
      }
    ...
    updateProductionStatus: publicProcedure
      ...
      .mutation(async ({ input }) => {
        await db.updateSupplierProductionStatus(input.confirmationId, input.productionStatus);
        return { success: true };
      }),

    updateDeliverySchedule: publicProcedure
      .input(z.object({
        confirmationId: z.number(),
        dailySchedule: z.record(z.string(), z.number()),
      }))
      .mutation(async ({ input }) => {
        await db.updateSupplierDailySchedule(input.confirmationId, input.dailySchedule);
        return { success: true };
      }),
  }),
});
...
// NOTE: New supplier portal endpoints added manually
// getDashboardStats and getNotifications should be inserted before getMaterials in supplierPortal router
